...
// ─── Catálogos realistas basados en documentos de JJ SeaFoods ───────────────
...
const proveedores = [
  ...
  { nombre: 'Joffre Copete',    ruc: '0891751842001', telefono: '0992012968', direccion: 'Esmeraldas' },
];

// Distribución realista: Tuna y Dorado dominan
const especies = [
...
// Selección ponderada de especie según peso de probabilidad
...
async function insertarRecepcion(client, { fecha, hora, provId, procedencia, responsable, numProductos, recCounter, alertaForzada }) {
  ...
  for (let p = 0; p < numProductos; p++) {
    ...
    const numP    = rand(1, 8);
    const pesos   = Array.from({ length: 10 }, (_, i) =>
      i < numP ? rand(esp.pesoMin, esp.pesoMax) : null
    );
    ...
    await client.query(
      ...
      [recId, esp.producto, talla, calidad,
       ...pesos,
       pesoTotal, descuento, pesoNeto]
    );
  }

  // Condiciones de transporte — alertaForzada asegura algunas alertas hoy
...
async function seed() {
  ...
  try {
    ...
    // ── HOY: 8-12 recepciones con datos ricos ─────────────────────────────
    ...
    for (let i = 0; i < horasHoy.length; i++) {
      ...
      await insertarRecepcion(client, {
        ...
        recCounter:   recCounter++,
        alertaForzada: i < 3,   // primeras 3 recepciones pueden tener alertas
      });
    }

    console.log(`\n✅ Seed completado exitosamente.`);
    console.log(`   Total recepciones insertadas: ${recCounter - 1}`);
